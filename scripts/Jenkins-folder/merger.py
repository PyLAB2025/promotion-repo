...
def fetch_branches(file_path, lower_env, update_lower_env, new_branch_created, higher_env, new_version=None):
    ...
    if lower_env == 'dev2' and lower_branch!= 'X':
        ...
        print(lower_branch, higher_branch)
...
def create_github_branch(github_url, base_branch, new_branch):
    ...
    try:
        ...
        if not github_token:
            raise EnvironmentError("GIT_TOKEN environment variable not set")
        
        parsed = urllib.parse.urlparse(github_url)
        clean_url = f"https://{github_token}@{parsed.hostname}{parsed.path}"

		# Update origin to include token
        subprocess.run(['git', 'remote', 'set-url', 'origin', clean_url], cwd=new_branch_dir, check=True)
        ...
        subprocess.run(['git', 'config', 'user.email', 'pylab2025@gmail.com'], cwd=new_branch_dir, check=True, timeout=30)
        subprocess.run(['git', 'config', 'user.name', 'PyLAB2025'], cwd=new_branch_dir, check=True, timeout=30) 
        subprocess.run(['git', 'add', '.'], cwd=new_branch_dir, check=True, timeout=30)
        subprocess.run(
            ['git', 'commit', '-m', f'Initialize {new_branch}: Clean  environment folders'],
            cwd=new_branch_dir, check=True, timeout=30
        )
                
        result = subprocess.run(['git', 'push', 'origin', new_branch], cwd=new_branch_dir, check=True, timeout=30)
        ...
        subprocess.run(['git', 'clone', '-b', 'master', github_url, master_dir], check=True, timeout=30)        
        
        # 🔐 Set authenticated remote for master as well
        subprocess.run(['git', 'remote', 'set-url', 'origin', clean_url], cwd=master_dir, check=True)
        ...
        subprocess.run(['git', 'config', 'user.email', 'pylab2025@gmail.com'], cwd=master_dir, check=True, timeout=30)
        subprocess.run(['git', 'config', 'user.name', 'pyLAB2025'], cwd=master_dir, check=True, timeout=30)
        subprocess.run(['git', 'add', 'meta-sheet.xlsx'], cwd=master_dir, check=True, timeout=30)
        subprocess.run(['git', 'commit', '-m', f'Add {new_branch} to meta-sheet'], cwd=master_dir, check=True, timeout=30)
        subprocess.run(['git', 'push', 'origin', 'master'], cwd=master_dir, check=True, timeout=30)
